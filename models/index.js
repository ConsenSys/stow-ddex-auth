...
const fs = require('fs');
const isTesting = process.env.LSTOWIS_TESTING;

const name = isTesting ? process.env.STOW_DB_TEST_NAME : process.env.STOW_DB_NAME;
const user = isTesting ? process.env.LINNIA_DB_TEST_USERNAME : process.env.STOW_DB_USERNAME;
...
const connectToDatabase = options => sequelize.sync(options).then(() => models);
